...
public class TagPrint
{
    ...
    public string g_sLabelFontName//              라벨 출력용 폰트 이름
    {
        get
        {
            WizCommon.INI_GS ini = new WizCommon.INI_GS();
...
public class WizWorkLib
{
    ...
    public DataTable GetCode(CodeTypeClss nType)
    {
        ...
        DataTable dt = WizCommon.DataStore.Instance.ProcedureToDataTable(sCode, null, false);
        return dt;
    }
    ...
    //proName과 같은 이름의 프로세스가 실행중이면 활성화 시켜준다.
    public bool ReturnProcessRunStop(string proName)
    {
        foreach (Process process in Process.GetProcesses())
        {
            if (process.ProcessName.ToUpper().Equals(proName.ToUpper()))
            {   // 활성화
    ...
    public bool ReturnKillRunningProcess(string proName)
    {
        foreach (Process process in Process.GetProcesses())
        {
            if (process.ProcessName.ToUpper().Equals(proName.ToUpper()))
    ...
    public void SetComboBox(ComboBox cbx, DataTable dt, List<WizCommon.CB_IDNAME> list_cbx)
    {
        //사이즈 조절
        ...
        if (dt != null)
        {
            ...
            {
                WizCommon.CB_IDNAME ci = new WizCommon.CB_IDNAME();
                ci.cbName = cbx.Name;
                ci.Idx = c;
        ...
        if (list_cbx != null)
        {
            foreach (WizCommon.CB_IDNAME ci in list_cbx)
            {
    ...
    /// <returns></returns>
    public string GetComboValue(ComboBox cbx, List<WizCommon.CB_IDNAME> list_cbx)
    {
        if (cbx.Items.Count == 0)
    ...
    //public int GetComboIndex(ComboBox cbx, List<WizCommon.CB_IDNAME> list_cbx)
    //{
    //    if (cbx.Items.Count == 0)
    //    { return 0; }
    //    int i = cbx.SelectedIndex;
    ...
    public string FindComboBoxIdx(ComboBox cbx, List<WizCommon.CB_IDNAME> list_cbx, string DepartID)
    {
        if (cbx.Items.Count == 0)
        { return ""; }
        foreach (WizCommon.CB_IDNAME ci in list_cbx)
    ...
    public string FindComboBoxID(ComboBox cbx, List<WizCommon.CB_IDNAME> list_cbx)
    {
        if (cbx.Items.Count == 0)
        { return ""; }
        int i = cbx.SelectedIndex;
    ...
    public void btnRowUp(DataGridView dgv, int colidx = 0)
    {
        if (dgv.Rows.Count > 0)
        {
            ...
            {
                int iSelRow = 0;
                iSelRow = dgv.SelectedRows[0].Index;
                if (iSelRow == 0) return;
                for (int k = iSelRow - 1; k >= 0; k--)
                {
                    if (dgv.Rows[k].Visible)
                    {
                        dgv[colidx, k].Selected = true;
                        break;
                    }
                }
            }
    ...
    public void btnRowDown(DataGridView dgv, int colidx = 0)
    {
        if (dgv.Rows.Count > 0)
        {
            if (dgv.SelectedRows.Count > 0)
            {
                ...
                for (int k = iSelRow + 1; k < dgv.Rows.Count; k++)
                {
                    if (dgv.Rows[k].Visible)
                    {
                        dgv[colidx, k].Selected = true;
    ...
    //가로 이동 버튼 좌, 
    public int btnColLeft(DataGridView dgv, int z)//그리드뷰, , 현재 보여지는 ColIndex
    {
        if (dgv.Columns.Count - 1 > z - 1 && z - 1 >= 0)
        {
            for (int i = z - 1; i >= 0; i--)
            {
    ...
    //가로 이동 버튼 우
    public int btnColRight(DataGridView dgv, int z)//그리드뷰, 현재 보여지는 ColIndex
    {
        if (dgv.Columns.Count - 1 > z + 1)
        {
            for (int i = z + 1; i < dgv.Columns.Count - 1; i++)
            {
                if (dgv.Columns[i].Visible)
                {
                    dgv.FirstDisplayedScrollingColumnIndex = i;
    ...
    public string MakeDateTime(string strType, string Date)
    {
        ...
        else if (strType.ToUpper() == "YYMMDD")
        {
            ...
            {
                value = Date.Substring(2, 2) + "-" + Date.Substring(4, 2) + "-" + Date.Substring(6, 2);
            }
        }
        ...
        else if (strType.ToUpper() == "YYYYMMDDHHMMSS")
        {
            value = Date.Substring(0, 4) + "-" + Date.Substring(4, 2) + "-" + Date.Substring(6, 2) + " " +
                Date.Substring(8, 2) + ":" + Date.Substring(10, 2) + ":" + Date.Substring(12, 2);
        }
        else if (strType.ToUpper() == "YYYYMMDDHHMM")
        {
            value = Date.Substring(0, 4) + "-" + Date.Substring(4, 2) + "-" + Date.Substring(6, 2) + " " +
                Date.Substring(8, 2) + ":" + Date.Substring(10, 2);
        }
        return value;
    }
    ...
    private bool PrinterStatusCheck(string printerName)
    {
    ...
    private bool PrintCheckOnOff(string sPrinter)
    {
        ...
        bool blStatusOn = false;
        string printerName = "";
        foreach (ManagementObject printer in searcher.Get())
        {
            printerName = printer["Name"].ToString().ToLower();
            if (printerName.Equals(sPrinter.ToLower()))
            {
    ...
    // 천마리 콤마, 소수점 버리기
    public string stringFormatN0(object obj)
    {
        if (obj == null) { return ""; }
    ...
    // 천마리 콤마, 소수점 두자리
    public string stringFormatN2(object obj)
    {
        if (obj == null) { return ""; }
    ...
    // 천마리 콤마, 소수점 자릿수 설정
    public string stringFormatNDigit(object obj, int num)
    {
        if (obj == null) { return ""; }
    ...
    // 데이터피커 포맷으로 변경
    public string DatePickerFormat(string str)
    {
        string result = "";
    ...
    public void Delay(int delayTime) //2021-11-10 딜레이 추가
    {
        ...
        {
            Application.DoEvents(); currentTime = DateTime.Now;
        }
    }

    public double ConvertDoubleN4(double dou)
    {
        double result;

        result = Math.Round(dou, 4);
