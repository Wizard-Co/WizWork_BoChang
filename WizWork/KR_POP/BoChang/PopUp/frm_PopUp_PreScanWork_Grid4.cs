...
namespace WizWork
{
    public partial class frm_PopUp_PreScanWork_Grid4 : Form
    {
        ...
        public List<string> lstChildLabelList = new List<string>();
        public List<string> lstChildArticleIDList = new List<string>();
        public List<string> lstChildArticleList = new List<string>();

        Dictionary<string, List<string>> PLChildArticleID = new Dictionary<string, List<string>>();     //작업지시가 다르고 하위품이 다른 경우 지시에 맞는 하위품이 들어가기 위한 Dic 2023-04-07
        //Dictionary<string, string> UseRealChildQty = new Dictionary<string, string>();       //하위품의 사용량을 판단하기 위한 Dic
        ...
        public frm_PopUp_PreScanWork_Grid4(string strProcessID, string strMachineID, string strLabelID) //2021-05-12
        {
            ...
            m_MachineID = strMachineID;
            m_LabelID = strLabelID;
            SetScreen();  //TLP 사이즈 조정strProcessID
        }
        ...
        private void CheckMtrArticleID()
        {
            ...
            for (int i = 0; i < GridData2.Rows.Count; i++)
            {
                ...
                Dictionary<string, object> sqlParameter = new Dictionary<string, object>();
                sqlParameter.Add("ArticleID", GridData2.Rows[i].Cells["BuyerArticleNo"].Value.ToString());
                DataTable dt = DataStore.Instance.ProcedureToDataTable("xp_WizWork_ArticleGrpLOTID", sqlParameter, false);
            ...
            try
            {
                if (e.KeyChar == (char)13)
                {
                    ...
                    if (BarcodeEnter())
                    {
                        ...
                        for (int i = 0; i < GridData2.Rows.Count; i++)
                        {
                            if (m_ArticleID == GridData2.Rows[i].Cells["BuyerArticleNo"].Value.ToString().Trim())
                            {
                        ...
                        //for(int i =0; i < GridData2.Rows.Count; i++)
                        //{
                        //    if (GridData2.Rows[i].Cells["Label"].ToString() == "" && GridData2.Rows[i].Cells["NewProductYN"].ToString() =="N")
                        //    {
                        //        btnOK.Enabled = false;
                        //        return;
                        //    }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        if ((m_MtrExceptYN == "N" && m_PCMtrExceptYN.Equals("")) || m_PCMtrExceptYN.Equals("N"))//예외처리YN : 예외처리 아닐때
                        {
                            string[] sProTerm = Frm_tprc_Main.gs.GetValue("Work", "ProTerm", "ProTerm").Split('|');//배열에 공정별 조건 넣기
                            foreach (string ProTerm in sProTerm)
                            {
                        ...
                        if ((m_MtrExceptYN.Equals("N") && m_PCMtrExceptYN.Equals("")) || m_PCMtrExceptYN.Equals("N"))
                        {
                            if ((m_FIFOExceptYN.Equals("N") && m_PCFIFOExceptYN.Equals("")) || m_PCFIFOExceptYN.Equals("N")) 
                            {
                                ...
                                sqlParameter2.Add("BarCodeList", strBarCode);
        ...
        private bool Find_BOM_Child()
        {
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    if (m_ArticleID == dr["CHildArticleID"].ToString().Trim())
                    {
                        Wh_Ar_ChildArticleID = dr["CHildArticleID"].ToString().Trim();
                    }
                }
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lstChildLabelList.Clear();
                    lstChildArticleList.Clear();
                    lstChildArticleIDList.Clear();

                    Frm_tprc_Main.lstStartLabel = new List<string>();
                    for (int i = 0; i < GridData2.Rows.Count; i++)
                    {
                       Frm_tprc_Main.lstStartLabel.Add(GridData2.Rows[i].Cells["Label"].Value.ToString());

                       lstChildLabelList.Add(GridData2.Rows[i].Cells["Label"].Value.ToString());
                       lstChildArticleList.Add(GridData2.Rows[i].Cells["Article"].Value.ToString());
                       lstChildArticleIDList.Add(GridData2.Rows[i].Cells["BuyerArticleNo"].Value.ToString());
