...
namespace WizWork
{
    public partial class frm_PopUp_PreScanWork2 : Form
    {
        ...
        private string m_FIFOExceptYN = "";      //선입선출 예외처리 
        ...
        public List<string> lstArticleList = new List<string>();
        ...
        public List<string> lstArticleListMove = new List<string>();
        ...
        List<string> InstIDList = new List<string>(); //2023-04-09 작업지시가 서로 다른 작업을 같이 시작했을 경우 작업지시 번호
        List<string> InstDetSeqList = new List<string>(); //2023-04-09 작업지시가 서로 다른 작업을 같이 시작했을 경우 작업지시번호순서
        ...
        private string m_PCMtrExceptYN = "";      //2023-12-07 공정별 예외처리 체크용도
        private string m_PCFIFOExceptYN = "";     // 2024-03-19 공정별 선입선출 예외처리 체크용도
        ...
        public frm_PopUp_PreScanWork2(string strProcessID, string strMachineID, string strLabelID, List<string> lstLabelList, List<float> lstQty, List<string> lstArticleList, List<string> ListChildLabelID, List<string> ListChildArticle, string strProcess)
        {
            this.lstLabelList = lstLabelList;
            this.lstQty = lstQty;
            this.lstArticleList = lstArticleList;
        ...
        public frm_PopUp_PreScanWork2(string strProcessID, string strMachineID, string strLabelID, List<string> lstLabelList, List<float> lstQty, List<string> ListChildLabelID, List<string> ListChildArticle, List<string> lstLabelListMove, List<float> lstQtyMove, string strProcess)
        {
            this.lstLabelList = lstLabelList;
            this.lstQty = lstQty;
        ...
        public frm_PopUp_PreScanWork2(string strProcessID, string strMachineID, string strLabelID, List<string> lstLabelList, List<float> lstQty, List<string> lstArticleList, List<string> ListChildLabelID, List<string> ListChildArticle, string strProcess, List<string> InstIDList, List<string> InstDetSeqList)
        {
            this.lstLabelList = lstLabelList;
            this.lstQty = lstQty;
            this.lstArticleList = lstArticleList;
            this.ListChildLabelID = ListChildLabelID; //2021-11-30 하위라벨리스트 추가
            this.ListChildArticle = ListChildArticle; //2021-11-30 하위품번리스트 추가
            this.InstIDList = InstIDList;             //2023-04-09
            this.InstDetSeqList = InstDetSeqList;     //2023-04-09
        ...
        private void frm_PopUp_PreScanWork_Load(object sender, EventArgs e)
        {
            ...
            GridCount = ListChildLabelID.Count; //이미 스캔한 라벨은 다시 리스트 안 넣기 위해 초기 그리드 수 저장
        ...
        private void setPreScanLabel()
        {
            try
            {
                ...
                {
                    if (ListChildLabelID[i].ToString() != "") 
                    {
                        GridData2.Rows.Add((GridData2.Rows.Count + 1).ToString()// 순번
                                                   , ListChildLabelID[i]
                                                   , ListChildArticle[i]
                                                   , "-"
                                                   , "-"
                ...
                if (GridData2.Rows.Count > 0) 
                {
                    ...
                    {
                        ListLabelID.Add(GridData2.Rows[i].Cells["Label"].Value.ToString());
                    }                
                }
            }
            catch(Exception ex)
        ...
        private string CheckLabelID(string strBarCode)
        {
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    m_FIFOExceptYN = Lib.CheckNull(dr["OutwareExceptYN"].ToString()); //선입선출 예외 처리                                                               
            ...
            try
            {
                if (e.KeyChar == (char)13)
                {
                    ...
                    if (BarcodeEnter())
                    {
                        ...
                        ListLabelID.Add(txtBarCodePreScan.Text); //2023-12-15
                    }
        ...
        private bool BarcodeEnter()
        {
            ...
            { 
                ...
                for (int i = 0; i < ListChildArticleID.Count; i++ ) 
                { 
                    if(ListChildArticleID[i].ToString().Trim() == m_ArticleID)
                    {
                        x++;
                    }
                    else
        ...
        private bool BarCodeCheck(string strBarCode)
        {
            ...
            try
            {
                if (m_Process.Contains("외주") == true) //2023-01-16 삼익SDT는 외주를 도금으로 처리함
                {
                    YLabelOK = false;
                ...
                else
                {
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        //선입선출 2023-12-06 입고일자로 기준으로 수정
                        //if ((m_MtrExceptYN.Equals("N") && m_PCMtrExceptYN.Equals("")) || m_PCMtrExceptYN.Equals("N"))
                        //{
                        //    if ((m_FIFOExceptYN.Equals("N") && m_PCFIFOExceptYN.Equals("")) || m_PCFIFOExceptYN.Equals("N"))
                        //    {
                        ...
                        //선입선출 2023-12-06 입고일자로 기준으로 수정
                        //바코드 스캔 되어 있는 바코드는 빼고 선입선출 해야 됨
        ...
        private bool Find_BOM_Child()
        {
            try
            {
                ...
                Dictionary<string, object> sqlParameter = new Dictionary<string, object>();
                sqlParameter.Clear();
                sqlParameter.Add("sInstID", Frm_tprc_Main.g_tBase.sInstID);
                sqlParameter.Add("sInstSeq", Frm_tprc_Main.g_tBase.sInstDetSeq);
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ListChildArticleID.Add(dr["CHildArticleID"].ToString().Trim());
                    }

                }
                

                ListChildArticleID = ListChildArticleID.Distinct().ToList();

                Wh_Ar_ChildArticleID = ListChildArticleID[0].ToString().Trim();
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                lstLabelList.Clear();
                lstQty.Clear();
                lstArticleList.Clear();

                lstLabelListMove.Clear(); //2021-10-07
                lstQtyMove.Clear();       //2021-10-07

                for (int i = GridCount; i < GridData2.Rows.Count; i++) //2021-09-08 역순으로 바꾸면서 i를 0부터 시작하게 수정 (i = 1 -> i = 0), 선 스캔 라벨은 더하지 않게 GridData2.Rows.Count - 1로 수정
                {
                    ...
                    lstArticleList.Add(GridData2.Rows[i].Cells["BuyerArticleNo"].Value.ToString());
        ...
        private void ProcessMtrExceptYN()
        {
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    m_PCFIFOExceptYN = dr["FirstInFirstOutYN"].ToString();
                }
            }
            catch (Exception excpt)
            {
