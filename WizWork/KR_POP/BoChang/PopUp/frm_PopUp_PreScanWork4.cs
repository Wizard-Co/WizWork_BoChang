...
namespace WizWork
{
    public partial class frm_PopUp_PreScanWork4 : Form
    {
        ...
        int BomArticleIDCount = 0; //하위품 수 판단하는 변수
        ...
        private bool BarCodeCheck(string strBarCode)
        {
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    m_StuffDate = dr["StuffDate"].ToString();
                    m_UnitClssName = dr["UnitClssName"].ToString();
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //else
                //{
                //    //입고일, 입고처리, 현재고량 라벨이 빈칸이면 시작되면 안됨
                //    // DB 임시 인서트.
                //    if (StartHandleWorking_Click() == true)
                //    {
                //        DialogResult = DialogResult.OK;
        ...
        {
            bool flag = true;
            if ((dgdMain.SelectedRows[0].Cells["LabelPrint"].Value.ToString().Equals("Y") && (txtBarCodePreScan.Text != null
                && !(txtBarCodePreScan.Text.ToString().Trim().Equals("")))))
            {
                return flag;
            }
            else if (dgdMain.SelectedRows[0].Cells["LabelPrint"].Value.ToString().Equals("N") && (dgdMain.SelectedRows[0] != null
            ...
            {
                Message[0] = "[시작 오류]";
                Message[1] = "라벨을 스캔하거나 선택하세요.";            //"선택된 라벨이 없습니다."; 2021-05-29 문구 변경
                WizCommon.Popup.MyMessageBox.ShowBox(Message[1], Message[0], 0, 1);
                return false;
            }
        }
        ...
        private bool StartHandleWorking_Click()
        {
            ...
            try
            {
                ...
                #region [사용 안함] 하위품이 하나일 때
                //Dictionary<string, object> sqlParameter2 = new Dictionary<string, object>();
                //sqlParameter2.Add("JobID", "");
                ////2021-05-31 스캔, 선택하는 구분하기 위해 조건 추가
                //if (dgdMain.SelectedRows[0].Cells["LabelPrint"].Value.ToString().Equals("Y"))
                //{
                //    sqlParameter2.Add("ChildLabelID", txtBarCodePreScan.Text);
                //    sqlParameter2.Add("ChildArticleID", m_ArticleID);
                //}
                //else
                //{
                ...
                //    sqlParameter2.Add("ChildArticleID", dgdMain.SelectedRows[0].Cells["ArticleID"].Value.ToString()); //2021-05-29 old : dgdMain.SelectedRows[0].Cells["ArticleID"].Value.ToString()
                //}
                ...
                #endregion
                #region 하위품이 여러개일때 → 하위품 그리드가 있을 경우(여러개일 가능성이 존재하여 이걸로 통일)
                //2022-05-20 원자재 예외출고 구분, 하위품 하나와 여러개일 경우 구분 2가지 경우 구분해야되서 조건 추가               
                //if (m_MtrExceptYN != "Y") 
                if ((m_MtrExceptYN.Equals("N") && m_PCMtrExceptYN.Equals("")) || m_PCMtrExceptYN.Equals("N"))
                {
