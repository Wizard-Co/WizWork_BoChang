...
namespace WizWork
{
    public partial class frm_tprc_Work_U_Move_Prd : Form
    {
        ...
        private List<string> lstArticleListMove = new List<string>();
        ...
        private double AnotherQtySum = 0;
        ...
        public frm_tprc_Work_U_Move_Prd(string strProcessID, string DayOrNightID, List<string> listChildLabelID, List<string> listChildArticle, List<string> listChildArticleID)
        {
            InitializeComponent();
            //updateJobID = JobID;
            m_ProcessID = strProcessID;
        ...
        private void SetCombox(List<string> ListChildLabelID, List<string> ListChildArticleID)
        {
            try
            {
                if (ListChildLabelID[0].ToString() != "") 
                {
                    Dictionary<string, object> sqlParameter = new Dictionary<string, object>();
                    sqlParameter.Add("LabelID", ListChildLabelID[0]);
                ...
                {
                    Dictionary<string, object> sqlParameter = new Dictionary<string, object>();
                    sqlParameter.Add("ArticleID", ListChildArticleID[0]);
                    DataTable dt = DataStore.Instance.ProcedureToDataTable("xp_Move_Custom_ArticleID", sqlParameter, false);
                                   ...
                                   double LabelPaper_Count, double LabelPaper_OneMoreQty, double QtyperBox)
        {
            try
            {
                ...
                //'--------------------------------------------------------------------------------
                // 첫 공정이다.
                if (LabelPrintYN == "Y" && lstLabelList.Count == 0 && Split_GBN == "YC")
                {
                    int mInstDetSeq = 0;
                    long nQtyPerBox = 0;
                ...
                if (Success)
                {
                    ...
                    if ((LabelPrintYN == "Y") && (Wh_Ar_LabelPrintYN == "Y") && (LabelPaper_Count > 0) && (Split_GBN == "YC"))
                    {
            ...
            try
            {
                ...
                {                    
                    ...
                    foreach (DataRow dr in dt2.Rows)
                    {
                        ...
                        if(dr["ArticleID"].ToString() == txtArticleID.Text.ToString().Trim())
                        {
        ...
        private void Form_Activate()
        {
            try
            {
                ...
                SetCombox(ListChildLabelID, ListChildArticleID); //2021-09-28 거래처 콤보박스 생성
        ...
        private bool BarCodeCheck(string strBarCode, string strArticleID)
        {
            ...
            try
            {
                if (strBarCode == "") //2022-03-07 원자재 예외출고 인 경우 빈 값으로 처리되어 조건 추가
                {
                    ...
                    sqlParameter.Add("InstID", Frm_tprc_Main.g_tBase.sInstID);
                    sqlParameter.Add("InstDetSeq", Frm_tprc_Main.g_tBase.sInstDetSeq);
                    DataTable dt = DataStore.Instance.ProcedureToDataTable("xp_prdWork_sGetSubItemInfo_PrdMove", sqlParameter, false);
        ...
        private void btnScanSum_Click(object sender, EventArgs e)
        {
            ...
            if (FPPSW.ShowDialog() == DialogResult.OK)
            {
                ...
                this.lstArticleListSum = FPPSW.lstArticleList;
                this.lstArticleListMove = FPPSW.lstArticleListMove;
        ...
        //2021-12-01 합계 스캔 후 DataGrid2에 추가
        private void BarcodeEnterBatch(List<string> ListChildLabelID)
        {
            ...
            try
            {
                //2021-12-01 합계 스캔 후 다시 일괄 스캔 할때 기존의 스캔은 삭제하고 다시 넣기 위해 추가
        ...
        //2021-12-02 합계 스캔 후 생산 가능량 계산 하기 
        ...
        private void chkBoxQty_CheckedChanged(object sender, EventArgs e)
        {
            if (((CheckBox)sender).Checked == true)
            {
                ((CheckBox)sender).Checked = false;
